...
class SquadBuilderPage extends StatefulWidget {
  //BookDetailsPage({@PathParam('bookId') this.bookId});
  const SquadBuilderPage({Key? key, @PathParam('gameId') required this.gameId})
      : super(key: key);
  final String gameId;
  @override
  _SquadBuilderPageState createState() => _SquadBuilderPageState();
}

class _SquadBuilderPageState extends State<SquadBuilderPage> {
        ...
        actions: [
          ElevatedButton(
              onPressed: () {
                ...
                print(_SquadBuilderPageState.activeTargets[0].toString());
              },
    ...
    return DragTarget<UserModel>(
        ...
        if (widget.activePlayer.uid == data?.uid) {
          setState(
            () {
              ...
              _SquadBuilderPageState.activeTargets[widget.arrayNum] = false;
              info['onBench'] = true;
              firebaseFirestore
                  .collection("users")
      ...
      onAccept: (data) {
        setState(() {
          ...
          _SquadBuilderPageState.activePlayers[widget.arrayNum] = true;
          info['onBench'] = false;
          firebaseFirestore
              .collection("users")
              .doc(widget.activePlayer.uid)
